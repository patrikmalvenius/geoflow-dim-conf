...
--récuperer la géométrie de bâtiment entier, update colonne geombuilding					
...
select  co.object_id as id, ST_GeomFromText('POLYGONZ(('||replace(
	replace(
		...
		array_to_string(
					array_append(
		arr, arr[1]
						), ',') ::text, ',', ''), '][', ','), '[', ''), ']', '')||'))',2154)  as lod22 from cjdb.city_object co, arra where type = 'Building' and co.object_id = arra.object_id group by co.object_id, arra.arr ) select lod22 from geom where geom.id = object_id and type = 'Building' )
...
(select object_id,   ST_GeomFromText('MULTIPOLYGONZ('||replace(replace(replace(replace(replace(array_to_string(array_agg( magic), ':'), ',',''), ']]:[[', ')),(('), '] [', ','), '[[', '(('), ']]','))')||')',2154) as lod22 from dynamite group by object_id) select lod22 from  geom where  city_object.object_id = geom.object_id );
